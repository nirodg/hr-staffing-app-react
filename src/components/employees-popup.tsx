...
export const dummyClient: UserDTO = {
  id: 1,
  username: "john.smith",
  firstName: "John",
  lastName: "Smith",
...
export default function ClientForm({
  ...
  const initialFormState: UserDTO = initialData ?? {
    id: 0,
    firstName: "",
    lastName: "",
    email: "",
  ...
  const [formData, setFormData] = React.useState<UserDTO>(initialFormState);
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const currentValue = initialData?.[name as keyof UserDTO] ?? "";
  ...
  const isFormValid = () => {
    return (
      ...
      (formData.available ?? false) !== false
    );
  };
